...
struct screen_fade_t {
  ...
  short          m_fade_flags;
  std::byte      m_r, m_g, m_b, m_a;
};

class base_client_t {
public:
  virtual int  init(create_interface_fn app_system_factory, create_interface_fn physics_factory,
                    global_vars* globals)                                 = 0;
  ...
  virtual int  in_key_event() = 0; // int event_code, button_code_t key_num, const char*
                                   // current_binding) = 0;
  ...
  virtual bool write_user_cmd_delta_to_buffer() = 0; // bf_write* buf, int from, int to, bool
                                                     // is_new_command) = 0;
  virtual void encode_user_cmd_to_buffer()      = 0; // bf_write& buf, int slot) = 0;
  virtual void decode_user_cmd_from_buffer()    = 0; // bf_read& buf, int slot) = 0;
  virtual void view_render()                    = 0; // vrect_t* rect) = 0;
  virtual void view_fade(screen_fade_t* sf)     = 0;
  virtual void set_crosshair_angle(const vector3_t& angle)                 = 0;
  ...
  virtual bool dispatch_user_message()             = 0; // int msg_type, bf_read& msg_data) = 0;
  virtual c_save_restore_data* save_init(int size) = 0;
  virtual void save_write_fields() = 0; // c_save_restore_data*, const char*, void*,
                                        // data_map_t*, type_description_t*, int) = 0;
  virtual void save_read_fields() = 0; // c_save_restore_data*, const char*, void*, data_map_t*,
                                       // type_description_t*, int) = 0;
  ...
  virtual void  write_save_game_screenshot_of_size(const char* file_name, int width, int height,
                                                   bool create_power_of_2_padded = false,
                                                   bool write_vtf                = false)   = 0;
  virtual bool  get_player_view2(view_setup_t& player_view)                  = 0;
  ...
  virtual void        setup_game_properties() = 0; // c_utl_vector<x_user_context>& contexts,
  ...
  virtual void  start_stats_reporting() = 0; // HANDLE handle, bool arbitrated) = 0;
  virtual void  invalidate_mdl_cache()  = 0;
  virtual void  in_set_sample_time(float frame_time) = 0;
  virtual void  reload_files_in_list()               = 0; // i_file_list* files_to_reload) = 0;
  ...
  virtual const char* translate_effect_for_vision_filter(const char* pch_effect_type,
                                                         const char* pch_effect_name)       = 0;
